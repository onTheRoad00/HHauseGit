...
- (void)draw {
    ...
    for (int i = 0; i < self.xValues.count; i++) {
        ...
        if (self.isShowPointLabel) {
            ...
            [self addLabelWithTitle:self.barToplableArray[i] atLocation:center andTag:i];
...
- (void)addLabelWithTitle:(NSString *)title atLocation:(CGPoint)location andTag:(NSInteger)tag {
    ...
    [button setTitleColor:K102GaryColor forState:UIControlStateSelected];
...
//    button.layer.backgroundColor = self.backColor.CGColor;
    ...
    if (tag == 0) {
         [button setBackgroundImage:[UIImage imageNamed:@"气泡左"] forState:UIControlStateSelected];
        button.x = location.x-3;
        button.y = 0;
      
    }else if (tag == 11){
         [button setBackgroundImage:[UIImage imageNamed:@"气泡右"] forState:UIControlStateSelected];
        button.x = location.x-60+3;
        button.y = 0;
       
    }else{
         [button setBackgroundImage:[UIImage imageNamed:@"气泡中"] forState:UIControlStateSelected];
        button.x = location.x-30;   // - label.width / 2;
        button.y = 0;
    ...
    button.width = labelSize.width+60;
    button.height = labelSize.height+26;
    
    button.tag = 200+tag;
    [button addTarget:self action:@selector(barDidClicked:) forControlEvents:UIControlEventTouchUpInside];
    
    UIView * lineView = [[UIView alloc]initWithFrame:CGRectMake(button.x, button.y+button.height, button.width, 1)];
