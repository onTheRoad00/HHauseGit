...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        self.xtextFont = [UIFont systemFontOfSize:12];
    }
    return self;
}
...
//        NSLog(@"%lu",(unsigned long)self.subviews.count);
    ...
    [self.buttonPointArray removeAllObjects];
    
    
      CGSize labelSize = CGSizeMake(_lablesizeWidth, 29);
    
    // 添加坐标轴Label
    for (int i = 0; i < self.xAxisTitleArray.count; i++) {
        NSString *title = self.xAxisTitleArray[i];
//        NSLog(@"self.xAxisTitleArray.count:%lu",(unsigned long)self.xAxisTitleArray.count);
        UILabel *label = [[UILabel alloc] init];
        label.text = title;
        label.font = self.xtextFont;
        label.textColor = self.textColor;
        
        NSDictionary *attr = @{NSFontAttributeName : self.xtextFont};
        CGSize xlabelSize = [title sizeWithAttributes:attr];
      
        
        
        label.x = (i + 1) * self.pointGap - xlabelSize.width / 2;
        label.y = self.height - labelSize.height;
        label.width = xlabelSize.width;
...
//    self.textHeight = textSize.height;
    ...
    CGFloat separateMargin = (self.height - self.topMargin - labelSize.height - self.xAxisTextGap - self.numberOfYAxisElements * separateHeight) / self.numberOfYAxisElements;
...
//        NSLog(@"self.titleLabelArray.count:%lu",(unsigned long)self.titleLabelArray.count);
...
//            NSLog(@"plot.pointArray.count:%lu",(unsigned long)plot.pointArray.count);
...
//            CGContextFillPath(context);
        ...
        if (plot.isChartViewFill) { // 画线，空白处填充
            ...
            for (int i = 0; i < plot.pointArray.count; i++) {
//                   NSLog(@"划线plot.pointArray.count:%lu",(unsigned long)plot.pointArray.count);
                NSNumber *value = plot.pointArray[i];
...
//            [[plot.lineColor colorWithAlphaComponent:0.35] set];
            ...
            for (int i = 0; i < plot.pointArray.count; i++) {
//                   NSLog(@"划线，之后线plot.pointArray.count:%lu",(unsigned long)plot.pointArray.count);
