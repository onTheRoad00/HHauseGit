...
- (void)commonInit {
    ...
    self.xtextFont = [UIFont systemFontOfSize:8];
...
- (void)draw {
    ...
    self.xAxisView.width = self.xAxisTitleArray.count * self.pointGap + 30;
//    self.scrollView.contentSize = self.xAxisView.frame.size;
    ...
    self.yAxisView.axisColor = [UIColor whiteColor];
    ...
    self.yAxisView.lablesizeWidth = self.lablesizeWidth;
    [self.yAxisView draw];
...
//    NSLog(@"self.xAxisView.plots:%@\n self.plots:%@",self.xAxisView.plots,self.plots);
...
//    self.xAxisView.chartViewFill = self.isChartViewFill;
    ...
    self.xAxisView.lablesizeWidth = self.lablesizeWidth ;
    self.xAxisView.textFont = self.textFont;
    self.xAxisView.xtextFont = self.xtextFont ;
//    
//    self.xAxisView.showTailAndHead = YES;
////    CGFloat scale =(self.xAxisTitleArray.count * self.gap + 100)/ (self.xAxisTitleArray.count * self.gap + 100);
////    self.pointGap = scale;
////    
...
-(void)test{
    ...
    NSLog(@"self.gap%f",self.gap);
    if (self.xAxisView.width == self.scrollView.width) {
        [UIView animateWithDuration:0.25 animations:^{
            
            self.xAxisView.width = self.xAxisTitleArray.count * self.gap + 30;
            NSLog(@"%f",self.xAxisView.width);
        }];
        ...
        CGFloat scale = self.scrollView.width / (self.xAxisTitleArray.count * self.gap + 30);
        
//        self.pointGap *= scale;
        NSLog(@"%f",self.pointGap);
