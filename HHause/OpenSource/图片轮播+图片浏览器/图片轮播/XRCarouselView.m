...
- (void)setImageArray:(NSArray *)imageArray{
    ...
    for (int i = 0; i < imageArray.count; i++) {
        if ([imageArray[i] isKindOfClass:[UIImage class]]) {
            ...
            [_images addObject:[UIImage imageNamed:@"顶部过渡"]];
...
- (void)setPageControlPosition {
    ...
    if (_pagePosition == PositionNone || _pagePosition == PositionBottomCenter)
        _pageControl.center = CGPointMake(self.width * 0.5, self.height - (_describeLabel.hidden? 10 : pageControlBottom));
...
- (void)layoutSubviews {
    ...
    _describeLabel.frame = CGRectMake(0, self.height - 40, self.width, 40);
