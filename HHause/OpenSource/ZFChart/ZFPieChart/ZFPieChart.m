...
//        [self addSubview:self.titleLabel];
        ...
        UIColor * color =[UIColor colorWithRed:145.0/255 green:145.0/255 blue:145.0/255 alpha:1];
...
//    _maxRadius = self.frame.size.width > self.frame.size.height ? self.frame.size.height : self.frame.size.width;
    ...
    _pieCenter = CGPointMake(_radius*1.8,_radius*1.8);
    _isShowDetail = YES;
    _isShowPercent = YES;
...
- (void)addUI{
    for (NSInteger i = 0; i < self.valueArray.count; i++) {
        //装载容器
        UIView * background = [[UIView alloc] initWithFrame:CGRectMake(kScreeWidth-ZZRQWIDTH-20,30+i*(_radius*2/self.valueArray.count+5),ZZRQWIDTH, _radius*2/self.valueArray.count+5)];
//        background.backgroundColor =[UIColor grayColor];
        ...
        //颜色View
        UIView * color = [[UIView alloc] initWithFrame:CGRectMake(0, 5, 14, 14)];
...
//        CGFloat gap = (SCREEN_WIDTH - CGRectGetMaxX(color.frame) - 10 - self.frame.size.width * 0.05 - 3 * width) / 2;
        ...
        //名称
        UILabel * name = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(color.frame) + 7, 5, width, 14)];
        ...
        //数值
        UILabel * value = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(name.frame) + 7, 5, width, 14)];
        value.text = _valueArray[i];
        value.font = [UIFont boldSystemFontOfSize:12.f];
